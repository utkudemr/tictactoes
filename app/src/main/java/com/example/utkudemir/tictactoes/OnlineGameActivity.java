...
public class OnlineGameActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        userPoint=getIntent().getExtras().get("user_point").toString();
        otherPlayerPoint=getIntent().getExtras().get("other_user_point").toString();
        ...
        if(requestType.equals("From")){
            myGameSign = "0";
            tvPlayer1.setText("Sıra sende");
            tvPlayer2.setText("Sıra sende");
            myRef.child("playing").child(playerSession).child("turn").setValue(userName);
            //setEnableClick(true);
        }else{
            myGameSign = "X";
            tvPlayer1.setText(otherPlayer + "\' sırası");
            tvPlayer2.setText(otherPlayer + "\' sırası");
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                try{
                    ...
                    if(value.equals(userName)) {
                        tvPlayer1.setText("Sıra Sende");
                        tvPlayer2.setText("Senin Sıran");
                        setEnableClick(true);
                        activePlayer = 1;
                    }else if(value.equals(otherPlayer)){
                        tvPlayer1.setText(otherPlayer + "\' sıra");
                        tvPlayer2.setText(otherPlayer + "\' sıra");
                        setEnableClick(false);
                        activePlayer = 2;
                    }
                }catch (Exception e){
    ...
    void CheckWinner(){
        ...
        if(winner == 1){
            int foo = Integer.parseInt(otherPlayerPoint);
            foo=foo+5;
            String otherPoint=Integer.toString(foo);
            ShowAlert(otherPlayer +" kazandı");
            myRef.child("users").child(otherPlayer).child("point").setValue(otherPoint);
        }else if(winner == 2){

            int foo2 = Integer.parseInt(userPoint);
            foo2=foo2+5;
            String otherPoint=Integer.toString(foo2);

            ShowAlert("Oyunu Kazandın");
            myRef.child("users").child(userName).child("point").setValue(otherPoint);
        }
        gameState = 2;
        }
    ...
    void ShowAlert(String Title){
        ...
        b.setTitle(Title)
                .setMessage("Menüye Yönlendiriliyorsun")
